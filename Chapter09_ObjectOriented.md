...
对象在实例化后，就可以把它当成一个世间万物里的实体对象。这个实体可以在电脑里充当一个角色的功能，比如对这个对象发出“吃饭”、“睡觉”等行为时，就使用 `obj.Eat()`、`obj.Sleep()` 这样类似的语法来让对象执行指定的行为。那么，这里的 `Eat` 和 `Sleep` 是通过绑定了一个实例来执行的，这样的方法称为实例方法

### 静态方法

如果一个方法跟某一个单独的对象无关，而是跟这个实例的基本属别来确定行为的，就称为**静态的**（static）
...
1. 写法：`[访问修饰符] 类型名(参数表列)`
2. 访问修饰符默认为 `private`
3. 构造器的用法和 `new` 关键字调用（实例化）
4. 构造器在不写的时候默认生成一个无参构造器；但如果写了至少一个构造器时，该类就不会自动生成默认的无参构造器了
5. 构造器可重载
6. 构造器可相互调用，但必须是在最开头就被调用，写法：`构造器声明 : this(被调用构造器的参数表列)`
7. 文档注释 `<param>`：修饰参数，表达参数的含义
8. 静态构造器：对一些静态字段在初始化的时候用的，不可以手动调用
   1. 写法：`static 类型名()`
   2. 不写访问修饰符，默认为 `private`

### **复制构造器**（Copy constructor）

将当前类型作为参数传递到构造器里，这个构造器叫复制构造器；但不能直接使用 `this` 接收，因为他俩还是一样的地址指向

### 终结器（Finalizer）

用于销毁对象时被系统自动调用，不可以手动调用
...
5. 注意，静态字段的初始化顺序，不要有依赖。比如说，如果后续才会初始化一个叫 `A` 的字段，就不要把这个字段放在 `B` 字段之后，否则如果初始化 `B` 字段时需要用 `A` 字段，而 `A` 字段没有初始化好，此时就会产生 `TypeInitializationException`（类型初始化异常）

### **属性**（Property）

Java 里 `getxxx` 和 `setxxx` 的封装模型
...
就是 C 里的**函数**（Function），以及为什么方法不叫函数，而叫方法
