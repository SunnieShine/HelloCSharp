...
允许使用小括号语法对多个变量赋值和取值的一种机制：

回顾一下 C# 4 提供的元组类型 `Tuple<>`，不好用的地方在于它必须要按照 C# 的语法来写，而且很笨重（引用类型），再加上取数值的时候需要使用 `Item1`、`Item2` 这样的名字，很丑，所以值元组（`ValueTuple<>`）诞生了

写法：`元组类型或var 变量名 = (数据列表)`，比如
...
这种写法等价于 `ValueTuple.Create` 方法的调用：
...
因为背后机制是用 `public` 的字段实现的，所以在取名的时候请建议使用帕斯卡命名法进行命名

### 对位赋值

使用值元组的语法对不同的变量或一些属性赋值：
...
类似 C# 3 的匿名类型，只要我们传入的是变量名而不是字面量的话，这个变量名就会自动被认为是这个值元组的当前元素名称，比如：
...
允许在一个类或结构里实现 `Deconstruct` 方法，来让这个类或对象拥有解构的行为：
...
public struct Student
{
    ...
    public void Deconstruct(out int age, out int isBoy) => (age, isBoy) = (Age, IsBoy);
}
```
...
允许变量声明的地方使用弃元关键字 `_` 来表达这个变量我们不要了（放弃掉，不用它）：
...
允许一个方法嵌套到某一个方法里，来表示这个方法本身只在某一个方法里执行和使用

## 二进制字面量

允许二进制形式的字面量：`0b11100011`

## 数字分隔符

允许对整数字面量用下划线分隔每一节内容，增强可读性：`0b11_10_00_11`；分隔符可以在任何时候和任何地方写（当然了，肯定不能写在 `0` 和 `b` 之间，对吧）

## `ref` 临时变量和 `ref` 返回

允许一个值类型对象以引用形式返回和定义变量，这样来减少内存分配，提高执行性能和效率

## 自定义异步返回类型

可以自己写一个异步类型出来，这样可以使用 `async` 语法来兼容你写的这个类型，但需要实现指定的方法和接口

## 更多的 Lambda 风格表达

现在属性、方法、索引器等成员都可以使用 Lambda 风格来表达返回结果了

## `throw` 表达式

内嵌到 `operator ?:` 和 `operator ??` 里，这样可以简化执行的代码

## 异步 `Main` 方法

允许 `Main` 方法是异步执行的：`async Task Main`

## `default` 表达式可以推断类型了

以前我们需要把字面量的类型写出来来表示这个类型的默认值；现在我们可以不写类型，只要能推断出类型就可以了：`int val = default`

## 允许泛型可以使用模式匹配

## `Span<T>` 类型和 `ref struct`（仅放在栈内存的类型）
...
## `in` 参数修饰符和 `readonly ref`

允许返回值和参数传参的时候只读，不可修改，类似 C 里的 `const`，但不同的是，C 的 `const` 是编译器层面的，而且你可以绕过；C# 的 `in` 是传指针过去，使得没办法修改；`readonly ref` 则是约束返回值传引用的基础上也不可修改

## `ref` 条件表达式

允许 `operator ?:` 返回 `ref` 结果

## 命名参数不一定只出现在末尾

C# 4 的命名参数只能放在方法的末尾，现在命名参数随时随地都可以用了

## `private protected` 访问修饰符

继承关系仅在项目内生效，别的外部程序调用 API 的时候将无法看到这类成员，即使是继承了

## `System.Enum` 和 `System.Delegate` 泛型约束
...
允许使用类似数组初始化器的模式初始化栈内存数组：`stackalloc[] { 1, 2, 3 }`

## `fixed` 字段

允许字段在初始情况下就固定下来长度，该字段将被改写为指针，但因为长度固定，所以内容可以直接使用，通过索引器的方式使用

## 自定义 `fixed` 语句声明

实现 `GetPinnableReference` 即可

## 值元组可以使用 `operator ==` 和 `operator !=` 了

## 允许把特性标记到属性上，直接指定到隐藏到背后的字段上